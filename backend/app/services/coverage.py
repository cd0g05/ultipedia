...
class CoverageModel:
    def __init__(self, *, saturated_at: float = 0.7, persistence=None) -> None:
    ...
    def hydrate(self) -> int:
        """Load persisted coverage rows into memory. Call once at startup."""
        if self._persistence is None:
            return 0
        n = 0
        for row in self._persistence.load_coverage():
            eid = UUID(str(row["entity_id"]))
            aspect = row["aspect"]
            if aspect not in ASPECTS:
                continue
            self._entity(eid)[aspect] = AspectCoverage(
                fill_score=float(row["fill_score"]),
    ...
    def record(self, entity_id: UUID, contribution: dict[str, float]) -> None:
        ...
        for aspect, amount in contribution.items():
            ...
            if self._persistence is not None:
                self._persistence.upsert_coverage(
                    entity_id, aspect, ac.fill_score, ac.confidence
                )
